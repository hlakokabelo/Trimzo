...
export const sign_up = async (data: SignupPayload): Promise<ApiResponse> => {
  try {
    const res = await axiosInstance.post(ROUTES.auth.signup, data);
    return {
      success: true,
      data: res.data,
    };
...
export const log_in = async (data: LoginPayload): Promise<ApiResponse> => {
  try {
    const res = await axiosInstance.post(ROUTES.auth.login, data);
    return {
      success: true,
      data: res.data,
    };
...
export const log_out = async (): Promise<ApiResponse> => {
  try {
    const res = await axiosInstance.post(ROUTES.auth.logout);
    return {
      success: true,
      data: res.data,
    };
